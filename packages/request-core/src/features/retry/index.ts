...
export class RetryFeature {
  ...
  async requestWithRetry<T>(
    ...
    if (
      retryConfig.backoffFactor !== undefined &&
      retryConfig.backoffFactor <= 0
    ) {
      throw new RequestError('Backoff factor must be positive')
    }
    if (
      retryConfig.jitter !== undefined &&
      (retryConfig.jitter < 0 || retryConfig.jitter > 1)
    ) {
    ...
    const backoff =
      retryConfig.backoffFactor && retryConfig.backoffFactor > 1
        ? retryConfig.backoffFactor
        : 1
    const jitter =
      retryConfig.jitter && retryConfig.jitter > 0 && retryConfig.jitter <= 1
        ? retryConfig.jitter
        : 0
    const shouldRetry =
      retryConfig.shouldRetry ?? this.defaultShouldRetry.bind(this)
    ...
    while (attempt < maxAttempts) {
      try {
        const attemptMessage = `🔄 [Retry] Making request (attempt ${
          attempt + 1
        }/${maxAttempts})`
        console.log(
        ...
        if (isLastAttempt || !shouldRetry(error, attempt)) {
          const finalMessage = `❌ [Retry] Request failed after ${
            attempt + 1
          } attempts`
          console.error(
            `${finalMessage}\n  URL: ${config.url}\n  Error: ${
        ...
        // 计算下次等待时间：可选指数退避加上抖动
        const delayBase =
          backoff === 1 ? baseDelay : baseDelay * Math.pow(backoff, attempt)
        const jitterDelta =
          jitter > 0 ? delayBase * (Math.random() * jitter) : 0
