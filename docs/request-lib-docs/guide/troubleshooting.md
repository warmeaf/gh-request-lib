...
export class HealthChecker {
  ...
  async checkApiHealth(): Promise<HealthReport> {
    ...
    try {
      ...
      await this.requestCore.get('/health', { timeout: 5000 })
      ...
      // 认证服务检查
      try {
        await this.requestCore.get('/auth/status', { timeout: 3000 })
...
class UserApi {
  ...
  async exportUsers(): Promise<Blob> {
    return this.requestCore.get<Blob>('/users/export', {
      timeout: 60000, // 60秒超时
      responseType: 'blob',
    })
  }
  ...
  async searchUsers(query: string): Promise<User[]> {
    return this.requestCore.get<User[]>('/users/search', {
      params: { q: query },
      timeout: 15000, // 15秒超时
    })
  }
...
class CacheDiagnostics {
  static async checkCacheHealth() {
    ...
    try {
      ...
      console.error('❌ 缓存功能不可用:', error.message)
      return
    }
    ...
    if ('storage' in navigator && 'estimate' in navigator.storage) {
      try {
        ...
        console.log('📊 存储使用情况:', {
          已使用: `${(usage / 1024 / 1024).toFixed(2)}MB`,
          配额: `${(quota / 1024 / 1024).toFixed(2)}MB`,
          使用率: `${usagePercentage.toFixed(1)}%`,
        })

        if (usagePercentage > 80) {
          console.warn('⚠️ 存储空间即将耗尽，建议清理缓存')
        }
      } catch (error) {
        console.error('❌ 无法获取存储信息:', error.message)
      }
    }
    ...
    if (stats) {
      console.log('📊 缓存统计:', {
        命中率: stats.hitRate ? `${(stats.hitRate * 100).toFixed(1)}%` : '未知',
        缓存大小: stats.size
          ? `${(stats.size / 1024 / 1024).toFixed(2)}MB`
          : '未知',
        项目数量: stats.itemCount || 0,
      })

      if (stats.hitRate && stats.hitRate < 0.3) {
...
export default defineConfig({
  ...
  build: {
    rollupOptions: {
      external: ['request-core', 'request-api'],
      output: {
        globals: {
  ...
  optimizeDeps: {
    include: ['request-core', 'request-api', 'request-imp-axios'],
  },
})
```
...
- 🔧 [进阶功能](/guide/advanced-features) - 深度定制功能
