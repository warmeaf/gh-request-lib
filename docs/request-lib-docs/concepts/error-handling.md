...
2. **智能错误分类**: 8 种错误类型自动推断
3. **详细上下文信息**: 包含 URL、方法、时间戳等
...
1. **NETWORK_ERROR** - 网络连接错误、DNS 解析失败、CORS 跨域错误
   - 建议：请检查网络连接或服务器是否可访问

2. **HTTP_ERROR** - 4xx、5xx HTTP 状态码错误
   - 401：认证失败，请检查 token 或登录状态
   - 404：请检查请求 URL 是否正确
   - 500：服务器错误，请稍后重试或联系管理员

3. **TIMEOUT_ERROR** - 请求超时、手动取消
   - 建议：可以尝试增加 timeout 值或检查网络状况
...
#### 认证错误处理
- 捕获 401 错误，自动刷新 token
- 刷新失败时跳转登录页

#### 业务错误转换
- 将 HTTP 错误转换为业务友好的错误信息
- 添加业务上下文和解决建议
